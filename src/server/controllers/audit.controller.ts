...
export class AuditController {
  static async list(request: NextRequest) {
    try {
      await requirePermission(request, "audit.manage");
  ...
  static async metrics(request: NextRequest) {
    try {
      ...
      if (query.resource) filters.push(sql`${schema.auditLogs.resource} = ${query.resource}`);
      if (query.action) filters.push(sql`${schema.auditLogs.action} = ${query.action}`);
      if (query.user) {
