...
export class ShipmentsRepository {
  async list(params: {
    page?: number;
    limit?: number;
    q?: string; // tracking number, order number, carrier
    status?: string;
    dateFrom?: string;
    dateTo?: string;
    ...
    if (params.q) {
      const like = `%${params.q}%`;
      filters.push(
        sql`(${schema.shipments.trackingNumber} ILIKE ${like} OR ${schema.shipments.carrier} ILIKE ${like} OR ${schema.orders.orderNumber} ILIKE ${like})`,
      );
    }
