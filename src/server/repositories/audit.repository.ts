...
export class AuditRepository {
  async list(params: { page?: number; limit?: number; userId?: string; user?: string; action?: string; resource?: string; dateFrom?: string; dateTo?: string; q?: string; sort?: "createdAt.desc" | "createdAt.asc" }) {
    ...
    if (params.action) filters.push(eq(auditLogs.action, params.action as any));
    if (params.resource) filters.push(eq(auditLogs.resource, params.resource));
    ...
    if (params.user) filters.push(sql`${users.email} ILIKE ${`%${params.user}%`}`);
