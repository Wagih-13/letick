...
class ProductsService {
      ...
      try {
        if (before) {
          ...
          if (priceDropped || backInStock) {
            const watchersAll = await storefrontWishlistRepository.listUserIdsByProduct(id);
            const watchers = priceDropped
  ...
  // ============== VARIANTS ==============
  async listVariants(productId: string): Promise<ServiceResult<any[]>> {
    try {
      const rows = await productsRepository.listVariants(productId);
      return success(rows);
    } catch (e: any) {
      return failure("LIST_PRODUCT_VARIANTS_FAILED", e?.message || "Failed to list product variants");
    }
  }
  ...
  async createVariant(
    ...
    try {
      const row = await productsRepository.createVariant(productId, input);
      return success(row);
    } catch (e: any) {
      return failure("CREATE_PRODUCT_VARIANT_FAILED", e?.message || "Failed to create product variant");
    }
  }
  ...
  async updateVariant(
    ...
    try {
      const row = await productsRepository.updateVariant(productId, variantId, patch);
      if (!row) return failure("NOT_FOUND", "Variant not found");
      return success(row);
    } catch (e: any) {
      return failure("UPDATE_PRODUCT_VARIANT_FAILED", e?.message || "Failed to update product variant");
    }
  }

  async removeVariant(productId: string, variantId: string): Promise<ServiceResult<{ id: string }>> {
    try {
      const ok = await productsRepository.removeVariant(productId, variantId);
      if (!ok) return failure("NOT_FOUND", "Variant not found");
      return success({ id: variantId });
    } catch (e: any) {
      return failure("DELETE_PRODUCT_VARIANT_FAILED", e?.message || "Failed to delete product variant");
    }
  }
}
