...
export function StorefrontHeader() {
  ...
  const hasControlPanel = useMemo(() => {
    ...
    if (typeof rawRoles === "string") list.push(rawRoles);
    else if (Array.isArray(rawRoles)) {
      for (const r of rawRoles) {
        if (!r) continue;
        if (typeof r === "string") list.push(r);
        else if (typeof r?.slug === "string") list.push(r.slug);
        else if (typeof r?.name === "string") list.push(r.name);
      }
    }
    const singleRole = (user as any)?.role;
    if (typeof singleRole === "string") list.push(singleRole);
    if ((user as any)?.isAdmin) list.push("admin");
    if ((user as any)?.isSuperAdmin) list.push("superadmin");
    return list.some((s) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!session) {
        // Guest state: hydrate wishlist snapshot without destroying existing guest cart
        try {
        ...
        // Only clear cart if this is a logout transition (previously authenticated)
        const prev = localStorage.getItem("last-auth-user") || "";
        const wasAuthed = !!prev && prev !== "guest";
        if (wasAuthed) {
          clearCart();
