...
export function SearchBar({ onClose, transparent }: SearchBarProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const to = inCategorySlug ? `/shop/${inCategorySlug}?q=${encodeURIComponent(q)}` : `/shop?q=${encodeURIComponent(q)}`;
  ...
  const select = (it: { type: string; value: any }) => {
    ...
    if (it.type === "category") {
      trackAnalytics("click_category", { q: query.trim(), id: it.value.id });
      router.push(`/shop/${it.value.slug}?q=${encodeURIComponent(query.trim())}`);
      setOpen(false);
      onClose?.();
      return;
    }
