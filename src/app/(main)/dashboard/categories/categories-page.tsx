  ...
  const columns = useMemo(
    () =>
      getCategoryColumns({
        ...
        onDelete: async (row) => {
          const yes = confirm(`Delete category ${row.name}?`);
          if (!yes) return;
          try {
            const res = await fetch(`/api/v1/categories/${row.id}`, { method: "DELETE" });
    ...
    <div className="@container/main flex flex-col gap-4 md:gap-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-6">
        <div className="rounded-lg border p-4 col-span-1">
          ...
          <div className="text-2xl font-semibold">{metrics?.totalCategories ?? total}</div>
        </div>
        <div className="rounded-lg border p-4 col-span-1">
          <div className="flex items-center justify-between">
          ...
          <div className="text-2xl font-semibold">{metrics?.activeCategories ?? "—"}</div>
        </div>
        <div className="rounded-lg border p-4 col-span-1">
          <div className="flex items-center justify-between">
          ...
          <div className="text-2xl font-semibold">{metrics?.inactiveCategories ?? "—"}</div>
        </div>
        <div className="rounded-lg border p-4 col-span-1">
          <div className="flex items-center justify-between">
            <div className="text-xs text-muted-foreground">Root categories</div>
            <FolderTree className="h-4 w-4 text-muted-foreground" />
          </div>
          <div className="text-2xl font-semibold">{metrics?.rootCategories ?? "—"}</div>
        </div>
        <div className="rounded-lg border p-4 col-span-1">
          <div className="flex items-center justify-between">
            <div className="text-xs text-muted-foreground">Child categories</div>
            <GitBranch className="h-4 w-4 text-muted-foreground" />
          </div>
          <div className="text-2xl font-semibold">{metrics?.childCategories ?? "—"}</div>
        </div>
        <div className="rounded-lg border p-4 col-span-1">
          <div className="flex items-center justify-between">
          ...
          <div className="text-2xl font-semibold">{metrics?.withImage ?? "—"}</div>
        </div>
      </div>
      {/* Toolbar */}
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={open} onOpenChange={(o) => (o ? setOpen(true) : (setOpen(false), resetForm()))}>
            ...
            <DialogContent className="sm:max-w-[520px] overflow-hidden">
              ...
              <div className="grid gap-3 py-2 max-h-[75vh] overflow-y-auto no-scrollbar modal-scrollbar pr-6 -mr-6">
