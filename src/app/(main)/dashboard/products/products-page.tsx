...
import { Plus, RefreshCcw, Package, CheckCircle2, Star, AlertTriangle, Ban, DollarSign, TrendingUp } from "lucide-react";
...
export default function ProductsPage() {
  ...
  async function fetchCategories() {
    try {
      const res = await fetch(`/api/v1/categories?page=1&limit=100`);
  ...
  async function fetchMetrics() {
    try {
      ...
      const res = await fetch(`/api/v1/products/metrics?${params.toString()}`);
      const data = await res.json();
  ...
  const columns = useMemo(
    () =>
      getProductColumns({
        ...
        onDelete: async (row) => {
          const yes = confirm(`Delete product ${row.name}?`);
          if (!yes) return;
          try {
            const res = await fetch(`/api/v1/products/${row.id}`, { method: "DELETE" });
            const data = await res.json();
    ...
    <div className="@container/main flex flex-col gap-4 md:gap-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-6">
        <div className="rounded-lg border p-4 col-span-1">
          ...
          <div className="text-2xl font-semibold">{metrics?.totalProducts ?? total}</div>
        </div>
        <div className="rounded-lg border p-4 col-span-1">
          <div className="flex items-center justify-between">
          ...
          <div className="text-2xl font-semibold">{metrics?.activeProducts ?? "—"}</div>
        </div>
        <div className="rounded-lg border p-4 col-span-1">
          <div className="flex items-center justify-between">
          ...
          <div className="text-2xl font-semibold">{metrics?.featuredProducts ?? "—"}</div>
        </div>
        <div className="rounded-lg border p-4 col-span-1">
          <div className="flex items-center justify-between">
        ...
        <div className="rounded-lg border p-4 col-span-1">
          ...
          <div className="text-2xl font-semibold">{metrics?.outOfStockProducts ?? "—"}</div>
        </div>
        <div className="rounded-lg border p-4 col-span-1">
          <div className="flex items-center justify-between">
        ...
        <div className="rounded-lg border p-4 col-span-1">
          <div className="flex items-center justify-between">
            <div className="text-xs text-muted-foreground">Units sold (30d)</div>
            <TrendingUp className="h-4 w-4 text-muted-foreground" />
          </div>
          <div className="text-2xl font-semibold">{Number(metrics?.unitsSold30d ?? 0)}</div>
        </div>
      </div>
      {/* Toolbar */}
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={open} onOpenChange={(o) => (o ? setOpen(true) : (setOpen(false), resetForm()))}>
            ...
            <DialogContent className="sm:max-w-[620px] overflow-hidden">
              ...
              <div className="max-h-[75vh] overflow-y-auto no-scrollbar modal-scrollbar pr-6 -mr-6">
