  ...
  const columns = useMemo(
    () =>
      getTemplateColumns({
        ...
        onDelete: async (row) => {
          const yes = confirm(`Delete template ${row.name}?`);
          if (!yes) return;
          try {
            const res = await fetch(`/api/v1/emails/templates/${row.id}`, { method: "DELETE" });
        ...
        <DialogContent className="sm:max-w-3xl max-h-[80vh] overflow-auto">
          ...
          <div className="grid gap-3">
            ...
            <div>
              ...
              <Textarea value={form.variables} onChange={(e) => setForm({ ...form, variables: e.target.value })} className="min-h-32 font-mono text-xs" />
            </div>
