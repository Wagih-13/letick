  ...
  const columns = useMemo(
    () =>
      getShippingMethodColumns({
        ...
        onDelete: async (row) => {
          const yes = confirm(`Delete shipping method ${row.name}?`);
          if (!yes) return;
          try {
            const res = await fetch(`/api/v1/shipping/methods/${row.id}`, { method: "DELETE" });
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={open} onOpenChange={(o) => (o ? setOpen(true) : (setOpen(false), resetForm()))}>
            ...
            <DialogContent className="sm:max-w-[620px]">
              <DialogHeader>
                <DialogTitle>{editing ? "Edit Shipping Method" : "Create Shipping Method"}</DialogTitle>
              </DialogHeader>
