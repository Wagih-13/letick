...
export default function AccountNotificationsPage() {
  ...
  const getTypeMeta = (type?: string) => {
    ...
    if (t.includes("order_ship") || t === "order_shipped") return { Icon: Truck, badge: "bg-blue-500/10 text-blue-600" };
    if (t.includes("order_deliver") || t === "order_delivered") return { Icon: Package, badge: "bg-emerald-500/10 text-emerald-600" };
    if (t.includes("order_")) return { Icon: Package, badge: "bg-primary/10 text-primary" };
    if (t.includes("promo") || t.includes("promotion")) return { Icon: Megaphone, badge: "bg-pink-500/10 text-pink-600" };
    if (t.includes("system") || t.includes("alert")) return { Icon: AlertTriangle, badge: "bg-amber-500/10 text-amber-600" };
    return { Icon: Bell, badge: "bg-muted text-foreground" };
  };
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <Input
            ...
            className="w-64"
          />
          <Button variant="outline" onClick={() => void fetchList({ reset: true })}>Search</Button>
        </div>
      </div>
